...
type
  ...
  private
    ...
    procedure ShowDestinoForm(ID: Integer);
  end;
...
constructor TFrameDestinos.Create(AOwner: TComponent);
var
  Pnl: TPanel;
  Lbl: TLabel;
...
begin
  ...
  // Card contenedor de la tabla — Borde limpio plano de 1px plano
...
// ══════════════════════════════════════════════════════════════
// PaintRounded — patrón estándar unificado de la suite
// ══════════════════════════════════════════════════════════════
procedure TFrameDestinos.PaintRounded(Sender: TObject);
var Pnl: TPanel;
begin
  Pnl := TPanel(Sender);
...
procedure TFrameDestinos.Refrescar(Sender: TObject);
var
  Q: TSQLQuery;
  Filtro: string;
  Row, ID: Integer;
begin
  ...
  Q := DM.AbrirQuery(
    ...
    'FROM destinos ' + Filtro + ' ORDER BY id DESC');
...
procedure TFrameDestinos.GridDrawCell(Sender: TObject; aCol, aRow: Integer;
...
procedure TFrameDestinos.GridMouseDown(Sender: TObject; Button: TMouseButton;
...
procedure TFrameDestinos.GridMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
...
procedure TFrameDestinos.HintTimerTick(Sender: TObject);
...
procedure TFrameDestinos.MostrarHintAccion(const Texto: string);
var R: TRect;
begin
...
procedure TFrameDestinos.GridDblClick(Sender: TObject);
var Row, ID: Integer;
begin
  Row := Grid.Row;
  if (Row < 1) or (Row >= Grid.RowCount) then Exit;
  ID := PtrInt(Grid.Objects[0, Row]);
  if ID > 0 then ShowDestinoForm(ID);
end;

procedure TFrameDestinos.btnNuevoClick(Sender: TObject);
begin
  ShowDestinoForm(0);
end;

procedure TFrameDestinos.GuardarClick(Sender: TObject);
begin
  FModalForm.ModalResult := mrOK;
end;

procedure TFrameDestinos.CancelarClick(Sender: TObject);
begin
  FModalForm.ModalResult := mrCancel;
end;

procedure TFrameDestinos.ToggleEstado(ID: Integer; EstadoActual: string);
...
begin
  ...
  try
    DM.EjecutarSQL('UPDATE destinos SET estado=''' + NuevoEstado +
...
procedure TFrameDestinos.ShowDestinoForm(ID: Integer);
var
  F: TForm;
  LblSection: TLabel;
  eNom, eDes: TEdit;
...
begin
  ...
  if not IsNew then begin
    Q := DM.AbrirQuery('SELECT nombre, descripcion FROM destinos WHERE id=' + IntToStr(ID));
    try
  ...
  try
    ...
    with TPanel.Create(F) do begin
      ...
      with TLabel.Create(F) do begin
        ...
        if IsNew then Caption := 'Nuevo destino' else Caption := 'Editar destino';
    ...
    LblSection.SetBounds(24, YPos, 300, 20); LblSection.Caption := 'Datos del Destino';
    ...
    // Bucle seguro de validación persistente
    while F.ShowModal = mrOK do begin
      ...
      try
        if IsNew then begin
          DM.EjecutarSQL('INSERT INTO destinos (nombre, descripcion, estado, ' +
          ...
          DM.EjecutarSQL('UPDATE destinos SET nombre=' +
            QuotedStr(UpperCase(Trim(eNom.Text))) +
...
destructor TFrameDestinos.Destroy;
begin
