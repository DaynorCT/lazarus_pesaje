...
constructor TFrameOrigenes.Create(AOwner: TComponent);
var
  Pnl: TPanel;
  Lbl: TLabel;
...
procedure TFrameOrigenes.Refrescar(Sender: TObject);
var
  Q: TSQLQuery;
  Filtro: string;
  Row, ID: Integer;
...
begin
  ...
  if (Row < 1) or (Row >= Grid.RowCount) then Exit;
  ID := PtrInt(Grid.Objects[0, Row]);
  if ID > 0 then ShowOrigenForm(ID);
end;

procedure TFrameOrigenes.btnNuevoClick(Sender: TObject);
begin
  ShowOrigenForm(0);
end;

procedure TFrameOrigenes.GuardarClick(Sender: TObject);
begin
  FModalForm.ModalResult := mrOK;
end;

procedure TFrameOrigenes.CancelarClick(Sender: TObject);
begin
...
procedure TFrameOrigenes.ShowOrigenForm(ID: Integer);
var
  F: TForm;
  LblSection: TLabel;
  eNom, eDes: TEdit;
...
begin
  ...
  try
    ...
    if F.ShowModal = mrOK then begin
      if Trim(eNom.Text) = '' then begin MostrarInfoDialogo('Origen', 'El nombre es obligatorio'); Exit; end;
      if DM.Transaccion.Active then DM.Transaccion.Rollback;
      DM.Transaccion.StartTransaction;
      try
        if IsNew then begin
        ...
        DM.Transaccion.Rollback;
        MostrarInfoDialogo('Origen', 'Error al guardar origen', dtError);
      end;
    end;
  finally
    F.Free;
