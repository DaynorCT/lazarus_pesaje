...
type
  ...
  private
    ...
    procedure ShowChoferForm(ID: Integer);
  end;
...
begin
  ...
  pnlNuevo.OnPaint := @PaintRounded;  // Tag=0 → sin borde (botón azul)
  ...
  // Tag=2 → borde blanco via PaintRounded (patrón estándar de toda la app)
...
// ══════════════════════════════════════════════════════════════
// PaintRounded — 3 casos (patrón estándar de toda la app):
//   Tag = 1 → borde CLR_INFO   (botón Cancelar del modal)
//   Tag = 2 → borde CLR_WHITE  (card contenedor tabla)
//   else    → sin borde        (botones Guardar, Agregar, etc.)
// ══════════════════════════════════════════════════════════════
procedure TFrameChoferes.PaintRounded(Sender: TObject);
var Pnl: TPanel;
begin
  Pnl := TPanel(Sender);
  ...
  end else if Pnl.Tag = 2 then begin
    // Card tabla — borde blanco suave con esquinas redondeadas
    Pnl.Canvas.Pen.Color := CLR_WHITE;
    Pnl.Canvas.Pen.Width := 1;
    Pnl.Canvas.Pen.Style := psSolid;
    Pnl.Canvas.RoundRect(1, 1, Pnl.Width - 1, Pnl.Height - 1, 8, 8);
  end else begin
    // Botones sin borde (Guardar, Agregar, etc.)
...
procedure TFrameChoferes.GridDrawCell(Sender: TObject; aCol, aRow: Integer;
...
begin
  ...
  if aCol = 5 then begin
    ...
    if Grid.Cells[4, aRow] = 'ACTIVO' then begin
  ...
  if aCol = 4 then begin
    ...
    Grid.Canvas.TextRect(aRect, aRect.Left, aRect.Top, Grid.Cells[4, aRow], Ts);
    Exit;
  end;
...
procedure TFrameChoferes.GridMouseDown(Sender: TObject; Button: TMouseButton;
...
procedure TFrameChoferes.GridMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var Col, Row, Zona, NewZone: Integer;
begin
  Grid.MouseToCell(X, Y, Col, Row);
...
procedure TFrameChoferes.HintTimerTick(Sender: TObject);
...
procedure TFrameChoferes.MostrarHintAccion(const Texto: string);
var R: TRect;
begin
...
procedure TFrameChoferes.GridDblClick(Sender: TObject);
var Row, ID: Integer;
begin
  Row := Grid.Row;
  if (Row < 1) or (Row >= Grid.RowCount) then Exit;
  ID := PtrInt(Grid.Objects[0, Row]);
  if ID > 0 then ShowChoferForm(ID);
end;
...
procedure TFrameChoferes.GuardarClick(Sender: TObject);
begin
  FModalForm.ModalResult := mrOK;
end;

procedure TFrameChoferes.CancelarClick(Sender: TObject);
begin
  FModalForm.ModalResult := mrCancel;
end;

procedure TFrameChoferes.ToggleEstado(ID: Integer; EstadoActual: string);
...
begin
  ...
  try
    DM.EjecutarSQL('UPDATE choferes SET estado=''' + NuevoEstado +
    ...
    DM.EjecutarSQL('UPDATE personas SET estado=''' + NuevoEstado +
      ...
      ''' WHERE id=(SELECT persona_id FROM choferes WHERE id=' + IntToStr(ID) + ')');
...
procedure TFrameChoferes.ShowChoferForm(ID: Integer);
var
  F: TForm;
  Lbl, LblSection: TLabel;
  eNom, ePat, eMat, eCI, eTel, eLic: TEdit;
  Nombre, ApPat, ApMat, CIStr, Tel, Lic: string;
  Q: TSQLQuery;
  IsNew: Boolean;
  YPos: Integer;
  DimID: Integer;
...
begin
  IsNew := ID = 0;
  Nombre := ''; ApPat := ''; ApMat := ''; CIStr := ''; Tel := ''; Lic := '';

  if not IsNew then begin
    Q := DM.AbrirQuery(
      'SELECT p.nombre, p.apellido_paterno, p.apellido_materno, p.ci, p.telefono, ' +
      'c.licencia FROM choferes c ' +
      'INNER JOIN personas p ON p.id = c.persona_id WHERE c.id = ' + IntToStr(ID));
    try
      if not Q.EOF then begin
        ...
        Lic    := UpperCase(Q.FieldByName('licencia').AsString);
      end;
    finally Q.Close; end;
  end;
  ...
  try
    ...
    with TPanel.Create(F) do begin
      ...
      with TLabel.Create(F) do begin
        ...
        if IsNew then Caption := 'Nuevo chofer' else Caption := 'Editar chofer';
    ...
    LblSection.SetBounds(24,YPos,300,20); LblSection.Caption := 'Datos personales';
    ...
    YPos := YPos + 28;
    eNom := MakeEditConBorde(YPos, 24, 180);  eNom.Text := Nombre;
    ePat := MakeEditConBorde(YPos, 212, 180); ePat.Text := ApPat;
    eMat := MakeEditConBorde(YPos, 400, 180); eMat.Text := ApMat;
    YPos := YPos + 48;

    MakeLabel(YPos, 24, 'Nro. Documento');
    MakeLabel(YPos, 212, 'Telefono');
    MakeLabel(YPos, 400, 'Licencia');
    YPos := YPos + 28;
    eCI  := MakeEditConBorde(YPos, 24, 180);  eCI.Text := CIStr;
    eTel := MakeEditConBorde(YPos, 212, 180); eTel.Text := Tel;
    eLic := MakeEditConBorde(YPos, 400, 180); eLic.Text := Lic;
    eLic.CharCase := ecNormal;
    YPos := YPos + 56;
    ...
    if F.ShowModal = mrOK then begin
      ...
      try
        if IsNew then begin
          ...
          DM.EjecutarSQL(
            'INSERT INTO choferes (persona_id,licencia,estado,fecha_creacion,fecha_modificacion) VALUES ('+
            IntToStr(DimID)+','+QuotedStr(Trim(eLic.Text))+',''ACTIVO'','+
          ...
          DM.EjecutarSQL('UPDATE personas SET nombre='+QuotedStr(Trim(eNom.Text))+
            ...
            ' WHERE id=(SELECT persona_id FROM choferes WHERE id='+IntToStr(ID)+')');
          DM.EjecutarSQL('UPDATE choferes SET licencia='+QuotedStr(Trim(eLic.Text))+
            ',fecha_modificacion='+QuotedStr(FechaHoraActual)+
...
destructor TFrameChoferes.Destroy;
begin
