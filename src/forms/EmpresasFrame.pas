...
begin
  ...
  while not Q.EOF do begin
    ...
    Grid.Cells[0, Row] := UpperCase(Q.FieldByName('nombre_empresa').AsString);
    Grid.Cells[1, Row] := UpperCase(Q.FieldByName('actividad_economica').AsString);
    Grid.Cells[2, Row] := Q.FieldByName('correo_electronico').AsString;
    Grid.Cells[3, Row] := UpperCase(Q.FieldByName('telefono').AsString);
    Grid.Cells[4, Row] := UpperCase(Q.FieldByName('estado').AsString);
    Grid.Cells[5, Row] := '';
...
var
  ...
  IsNew: Boolean;
  YPos: Integer;
  lblRemoveLogo, LblCargarLogo: TLabel;
  Stream: TMemoryStream;
  RawBytes: RawByteString;
  ...
  try
    ...
    with TPanel.Create(F) do begin
      ...
      with TLabel.Create(F) do begin
        ...
        if IsNew then Caption := 'Nueva empresa' else Caption := 'Editar empresa';
    ...
    LblSection.SetBounds(24,YPos,300,20); LblSection.Caption := 'Datos de la Empresa';
    ...
    eAct := MakeEditConBorde(YPos, 314, 260); eAct.Text := Actividad;
    YPos := YPos + 48;

    MakeLabel(YPos, 24, 'Correo electronico');
    MakeLabel(YPos, 314, 'Telefono');
    YPos := YPos + 28;
    eCorreo := MakeEditConBorde(YPos, 24, 280); eCorreo.CharCase := ecNormal; eCorreo.Text := Correo;
    eTel := MakeEditConBorde(YPos, 314, 260); eTel.Text := Telefono;
    YPos := YPos + 48;

    MakeLabel(YPos, 24, 'Logo');
    YPos := YPos + 28;

    // Panel de preview del logo
    ...
    // Botón "Cargar logo" — OnClick tanto en panel como en label
    ...
    FModalBtnLogo.Tag := 1;           // borde CLR_INFO via PaintRounded
    FModalBtnLogo.Cursor := crHandPoint;
    FModalBtnLogo.OnPaint := @PaintRounded;
    FModalBtnLogo.OnClick := @CargarLogoClick;  // también en el panel
    ...
    Lbl.Caption := 'JPG, PNG o WEBP. Tamaño máximo: 2MB.';
      ...
      try
        if IsNew then begin
          DM.EjecutarSQL('INSERT INTO empresas (nombre_empresa,actividad_economica,' +
            ...
            QuotedStr(Trim(eNom.Text))+','+QuotedStr(Trim(eAct.Text))+','+
            QuotedStr(Trim(eCorreo.Text))+','+QuotedStr(Trim(eTel.Text))+','+
            QuotedStr(FLogoBase64)+',''ACTIVO'','''+FechaHoraActual+''','''+FechaHoraActual+''')');
        end else begin
