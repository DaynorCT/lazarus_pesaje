...
begin
  ...
  // Card tabla — borde blanco plano 1px (igual que todos los frames ABM)
...
begin
  ...
  if aCol = 4 then begin
    ...
    if Grid.Cells[3, aRow] = 'ACTIVO' then begin
  ...
  if aCol = 3 then begin
    ...
    Grid.Canvas.TextRect(aRect, aRect.Left, aRect.Top, Grid.Cells[3, aRow], Ts);
    Exit;
  end;
...
var
  ...
  begin
    ...
    if ANumerico then Result.CharCase := ecNormal
    else Result.CharCase := ecUpperCase;
  end;
  ...
  try
    ...
    with TPanel.Create(F) do begin
      ...
      with TLabel.Create(F) do begin
        ...
        if IsNew then Caption := 'Nuevo vehículo' else Caption := 'Editar vehículo';
    ...
    LblSection.SetBounds(24,YPos,300,20); LblSection.Caption := 'Datos del Vehículo';
    ...
    YPos := YPos + 28;
    ePlaca := MakeEditConBorde(YPos, 24, 280);  ePlaca.Text := Placa;
    eTipo  := MakeEditConBorde(YPos, 314, 260); eTipo.Text := Tipo;
    YPos := YPos + 48;

    MakeLabel(YPos, 24, 'Tara (kg)');
    YPos := YPos + 28;
    eTara := MakeEditConBorde(YPos, 24, 160, True); eTara.Text := TaraStr;
    YPos := YPos + 56;
      ...
      try
        if IsNew then begin
          DM.EjecutarSQL('INSERT INTO vehiculos (placa,tipo_vehiculo,tara,estado,' +
            ...
            eTara.Text+',''ACTIVO'','''+FechaHoraActual+''','''+FechaHoraActual+''')');
        end else begin
          DM.EjecutarSQL('UPDATE vehiculos SET placa='+QuotedStr(UpperCase(Trim(ePlaca.Text)))+
