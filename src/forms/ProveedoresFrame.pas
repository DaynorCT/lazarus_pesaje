...
begin
  ...
  Q := DM.AbrirQuery(
    ...
    'WHERE 1=1 ' + Filtro + ' ORDER BY pr.id DESC');
  ...
  try
    ...
    with TPanel.Create(F) do begin
      ...
      with TLabel.Create(F) do begin
        ...
        if IsNew then Caption := 'Nuevo proveedor' else Caption := 'Editar proveedor';
    ...
    LblSection := TLabel.Create(F); LblSection.Parent := F;
    LblSection.SetBounds(24,YPos,300,20); LblSection.Caption := 'Datos de empresa';
    LblSection.Font.Size := 11; LblSection.Font.Style := []; LblSection.Font.Color := CLR_TEXT_HEADING;
    YPos := YPos + 33;
    ...
    YPos := YPos + 28;
    eEmp  := MakeEditConBorde(YPos, 24, 280);  eEmp.Text := Emp;
    eDesc := MakeEditConBorde(YPos, 314, 260); eDesc.Text := Desc;
    YPos := YPos + 56;
    ...
    if F.ShowModal = mrOK then begin
      ...
      try
        if IsNew then begin
          ...
          DM.EjecutarSQL(
            'INSERT INTO proveedores (persona_id,nombre_empresa,descripcion,estado,fecha_creacion,fecha_modificacion) VALUES ('+
            IntToStr(DimID)+','+QuotedStr(Trim(eEmp.Text))+','+QuotedStr(Trim(eDesc.Text))+',''ACTIVO'','+
