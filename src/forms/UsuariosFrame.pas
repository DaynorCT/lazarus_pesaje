...
begin
  ...
  // Tag=2 → borde blanco via PaintRounded (patrón consistente con toda la app)
...
begin
  ...
  while not Q.EOF do begin
    ...
    Grid.Cells[0, Row] := UpperCase(Q.FieldByName('ci').AsString);
    Grid.Cells[1, Row] := UpperCase(Trim(
      Q.FieldByName('nombre').AsString + ' ' +
      Q.FieldByName('apellido_paterno').AsString + ' ' +
      Q.FieldByName('apellido_materno').AsString));
    Grid.Cells[2, Row] := UpperCase(Q.FieldByName('email').AsString);
    Grid.Cells[3, Row] := UpperCase(Q.FieldByName('telefono').AsString);
    Grid.Cells[4, Row] := UpperCase(Q.FieldByName('rol').AsString);
    Grid.Cells[5, Row] := UpperCase(Q.FieldByName('estado').AsString);
...
begin
  ...
  for I := 0 to Grid.ColCount - 1 do
    if Grid.Cells[I, 0] = ColName then begin Result := I; Exit; end;
end;
...
begin
  ...
  if aCol = 6 then begin
    ...
    if Grid.Cells[5, aRow] = 'ACTIVO' then begin
...
procedure TFrameUsuarios.GridMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
...
begin
  ...
  if (FHoverRow <> Row) or (FHoverZone <> NewZone) then begin
    if (FHoverRow > 0) and (FHoverRow < Grid.RowCount) then Grid.InvalidateCell(6, FHoverRow);
    FHoverRow := Row; FHoverZone := NewZone;
    if (Row > 0) and (Row < Grid.RowCount) then Grid.InvalidateCell(6, Row);
...
begin
  ...
  case FHoverZone of
    1: if Grid.Cells[5, FHoverRow] = 'ACTIVO' then Texto := 'Desactivar' else Texto := 'Activar';
...
var
  ...
  begin
    ...
    if APassword then begin Result.PasswordChar := '*'; Result.CharCase := ecNormal; end;
  end;
  ...
  try
    ...
    with TPanel.Create(F) do begin
      ...
      with TLabel.Create(F) do begin
        ...
        if IsNew then Caption := 'Nuevo usuario' else Caption := 'Editar usuario';
    ...
    cmbRol.AutoSize := False; cmbRol.Style := csDropDownList;
    cmbRol.Font.Size := 12; cmbRol.Color := CLR_WHITE; cmbRol.Font.Color := CLR_TEXT;
    ...
    LblSection := TLabel.Create(F); LblSection.Parent := F;
    LblSection.SetBounds(24,YPos,300,20); LblSection.Caption := 'Datos de usuario';
    LblSection.Font.Size := 11; LblSection.Font.Style := []; LblSection.Font.Color := CLR_TEXT_HEADING;
    YPos := YPos + 33;
    ...
    eEmail.CharCase := ecNormal; eEmail.Text := Email;
    YPos := YPos + 48;

    Lbl := MakeLabel(YPos, 24, 'Contraseña');
    if not IsNew then Lbl.Caption := 'Contraseña (dejar en blanco para mantener)';
    YPos := YPos + 28;
      ...
      try
        if IsNew then begin
          ...
          DM.EjecutarSQL('INSERT INTO personas (nombre,apellido_paterno,apellido_materno,ci,telefono,correo,estado,fecha_creacion,fecha_modificacion) VALUES (' +
            ...
            QuotedStr(Trim(eCI.Text))+','+QuotedStr(Trim(eTel.Text))+','+QuotedStr(Trim(eEmail.Text))+
          ...
          DM.EjecutarSQL('INSERT INTO usuarios (persona_id,email,password_hash,rol,estado,fecha_creacion,fecha_modificacion) VALUES ('+
            IntToStr(DM.ObtenerUltimoID)+','+QuotedStr(Trim(eEmail.Text))+','+QuotedStr(Hash)+','+
            QuotedStr(cmbRol.Text)+',''ACTIVO'','''+FechaHoraActual+''','''+FechaHoraActual+''')');
        end else begin
...
begin
  // Cerrar modal si está abierto antes de destruir el frame
